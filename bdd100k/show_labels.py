...
def parse_args():
    ...
    parser.add_argument(
        "--no-attr", action="store_true", default=False, help="Do not show attributes"
    )
    parser.add_argument(
        "--no-lane", action="store_true", default=False, help="Do not show lanes"
    )
    parser.add_argument(
        "--no-drivable",
        action="store_true",
    ...
    parser.add_argument(
        "--drivable", action="store_true", default=False, help="Set drivable area mode"
    )
    parser.add_argument(
        "--lane-only", action="store_true", default=False, help="Show only lane lines"
    )
    parser.add_argument(
        "--target-objects",
        type=str,
...
def get_areas_v0(objects):
    return [o for o in objects if "poly2d" in o and o["category"][:4] == "area"]


def get_areas(objects):
    return [o for o in objects if "poly2d" in o and o["category"] == "drivable area"]


def get_lanes(objects):
    return [o for o in objects if "poly2d" in o and o["category"][:4] == "lane"]
...
def instance2color(instance):
    instance_colors = dict(
        [(i, (np.random.random(3) * 255).astype(np.uint8)) for i in np.unique(instance)]
    )
...
def convert_instance_rgb(label_path):
    ...
    Image.fromarray(seg_color).save(join(label_dir, label_name + "_train_color.png"))
    Image.fromarray(instance).save(join(label_dir, label_name + "_instance_id.png"))
...
def convert_drivable_rgb(label_path):
    ...
    Image.fromarray(seg_color).save(join(label_dir, label_name + "_drivable_color.png"))
...
class LabelViewer(object):
    ...
    def show_image(self):
        ...
        if "attributes" in label and self.with_attr:
            ...
            self.ax.text(
                ...
                90 * self.scale,
                "  scene: {}\nweather: {}\n   time: {}".format(
                    attributes["scene"], attributes["weather"], attributes["timeofday"]
                ),
        ...
        if self.with_box2d:
            [self.ax.add_patch(self.box2rect(b["box2d"])) for b in get_boxes(objects)]
    ...
    def next_image(self, event):
        if event.key == "n":
            ...
            return
        self.current_index = max(min(self.current_index, len(self.image_paths) - 1), 0)
        if self.show_image():
            plt.draw()
        else:
    ...
    def draw_drivable(self, objects):
        ...
        colors = (
            np.array([[0, 0, 0, 255], [217, 83, 79, 255], [91, 192, 222, 255]]) / 255
        )
        for obj in objects:
            if self.color_mode == "random":
                ...
                alpha = 1
            self.ax.add_patch(
                self.poly2patch(obj["poly2d"], closed=True, alpha=alpha, color=color)
            )
    ...
    def draw_lanes(self, objects):
        ...
        #                    [217, 83, 79, 255],
        #                    [91, 192, 222, 255]]) / 255
        colors = np.array([[0, 0, 0, 255], [255, 0, 0, 255], [0, 0, 255, 255]]) / 255
        for obj in objects:
            if self.color_mode == "random":
                ...
                alpha = 1
            self.ax.add_patch(
                self.poly2patch(obj["poly2d"], closed=False, alpha=alpha, color=color)
            )
    ...
    def draw_segments(self, objects):
        ...
        for obj in objects:
            ...
            if color_mode == "random":
                ...
                raise ValueError("Unknown color mode {}".format(self.color_mode))
            for segment in obj["segments2d"]:
                self.ax.add_patch(
                    self.poly2patch(segment, closed=True, alpha=alpha, color=color)
                )
...
class LabelViewer2(object):
    ...
    def write(self):
        ...
        while self.file_index < len(self.label_paths):
            ...
            out_name = (
                splitext(
                    split(self.label[self.frame_index - self.start_index]["name"])[1]
                )[0]
                + ".png"
            )
            out_path = join(self.out_dir, out_name)
    ...
    def draw_drivable(self, objects):
        ...
        for obj in objects:
            if self.color_mode == "random":
                ...
                color = (
                    (1 if obj["attributes"]["areaType"] == "direct" else 2) / 255.0,
                    (obj["id"] // 255) / 255,
                    (obj["id"] % 255) / 255.0,
                )
                alpha = 1
